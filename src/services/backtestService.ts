...
export async function fetchCandleCoverage(): Promise<CandleCoverage[]> {
  ...
  if (error) {
    ...
    const coverage = new Map<string, { earliestTime: string; latestTime: string; count: number }>();
    fallback.forEach((row: { instrument: string; granularity: string; time: string }) => {
      const key = `${row.instrument}-${row.granularity}`;
      const existing = coverage.get(key);
      if (existing) {
        existing.count++;
        if (row.time < existing.earliestTime) existing.earliestTime = row.time;
        if (row.time > existing.latestTime) existing.latestTime = row.time;
      } else {
        coverage.set(key, {
    ...
    return Array.from(coverage.entries()).map(([key, stats]) => {
...
export async function saveBacktestRun(
  ...
  run: {
    ...
    candleCount: number;
    candleSource?: 'historical' | 'synthetic';
  },
): Promise<BacktestRun> {
  const { data, error } = await supabase
    .from('backtest_runs')
    .insert({
      ...
      candle_source: run.candleSource ?? 'historical',
    })
    .select()
    .single();
...
export async function fetchBacktestRuns(userId: string): Promise<BacktestRun[]> {
  ...
  return (data ?? []).map(row => ({
    ...
    candle_source: row.candle_source ?? 'historical',
