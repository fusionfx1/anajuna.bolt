...
// ── Client-side cache (persists for the JS session) ───────────────────────────
...
/** Minutes until a UTC epoch ms from now (negative = in the past) */
...
interface UseNewsDataReturn {
  ...
  loading: boolean;
  error: string | null;
  /** Next upcoming high-impact event, or null */
...
export function useNewsData(): UseNewsDataReturn {
  ...
  const load = useCallback(async (force = false) => {
    // Serve from client cache unless forced or expired
  ...
  // Derive next upcoming high-impact event
  const now = Date.now();
