...
function NewsRow({ event }: { event: NewsEvent }) {
  ...
  const timeStr = event.utcMs !== null
    ? fmtTimeBangkok(event.utcMs)
    : 'All Day';
  ...
  return (
    <tr className={`border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors ${rowBg}`}>
      <td className="px-4 py-3 text-sm font-mono text-slate-400 whitespace-nowrap">
        {timeStr}
        {isLive && <span className="ml-1.5 text-[10px] text-red-400 font-semibold animate-pulse">● Live</span>}
        {isImminent && !isLive && (
      ...
      <td className="px-4 py-3">
        <p className={`text-sm font-medium ${
          event.impact === 'high' ? 'text-slate-100' : 'text-slate-300'
        }`}>
          {event.title}
        </p>
      </td>
...
function DayHeader({ label }: { label: string }) {
  return (
    <tr>
      <td
        colSpan={7}
        className="px-4 py-2 bg-slate-800/60 text-xs font-semibold text-slate-400 uppercase tracking-widest sticky top-0 z-10"
      >
        {label}
      </td>
    </tr>
...
export function NewsCalendar() {
  ...
  const now = Date.now();
  const nextImminent = events.find(
    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Asia/Bangkok time (UTC+7) · This week · {totalHigh} high-impact events
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <label className="flex items-center gap-2 cursor-pointer group">
            <div
              ...
              className={`relative w-8 h-4.5 rounded-full border transition-colors duration-200 ${
                highOnly
            ...
            <span className="text-xs font-medium text-slate-400 group-hover:text-slate-200 transition-colors select-none flex items-center gap-1">
              <Filter size={11} />
              High impact only
            </span>
          </label>
        ...
        <div className="flex items-center gap-3 bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-amber-500/80">
              {nextImminent.country} · {fmtTimeBangkok(nextImminent.utcMs)} BKK
              · in {minsUntil(nextImminent.utcMs)} min
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider bg-slate-900 border-b border-slate-800">
                  <th className="px-4 py-3 whitespace-nowrap">Time (BKK)</th>
      ...
      <div className="flex items-center gap-4 text-[10px] text-slate-600">
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-sm bg-red-500/30 border-l-2 border-red-500" />
          Live event
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-sm bg-amber-500/20 border-l-2 border-amber-500" />
          Within 30 min
        </span>
        <span className="flex items-center gap-1">
