...
- After **code-only** edits, prefer `npm run graphify:ast` from the project root so the graph stays aligned with the committed `GRAPH_REPORT.md` workflow (and to avoid a flaky `graphify update` exit on some Windows installs).
- Heavier outputs stay gitignored; only `graphify-out/GRAPH_REPORT.md` is meant for version control here.
